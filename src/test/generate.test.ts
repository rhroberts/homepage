...
describe('Static Site Generation', () => {
  ...
  it('generates all required HTML files', async () => {
    ...
    expect(await fs.access(resumeFile).then(() => true).catch(() => false)).toBe(true)
  })
  ...
  it('generates valid HTML with correct titles', async () => {
    ...
    const projectsHtml = await fs.readFile(join(distDir, 'projects', 'index.html'), 'utf-8')
    const resumeHtml = await fs.readFile(join(distDir, 'resume', 'index.html'), 'utf-8')
